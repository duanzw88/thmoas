...
static double timeTrial(const char *sort_function,void *data,int N)
{
	...
	else if(strcmp(sort_function,"Heap") == 0)
	{
		heap_sort(data,0,N-1,sizeof(double),cmp_double);
	}
...
int main(int argc,char *argv[])
{
	...
	// double heapT   = timeRandomInput("Heap",N,T);
	// printf("insertT = %f  shellT = %f selectT = %f mergeT = %f quickT = %f quick3T = %f heapT = %f\n",insertT,shellT,selectT,mergeT,quickT,quick3wayT,heapT);

	printf("检验快速排序\n");
	for(int i = 250;i < 10000000;i*=2)
	{
		double t = timeRandomInput("Quick",i,1);
